...
function tep_admin_jqmenu($filename, $sub_box_name, $target_window, $submenu = '') {
  ...
  //$dbquery = tep_db_query("select admin_files_name from " . TABLE_ADMIN_FILES . " where admin_files_name = '" . $filename . "'");
  $dbquery = tep_db_query("select admin_files_name from " . TABLE_ADMIN_FILES . " where FIND_IN_SET( '" . $login_groups_id . "', admin_groups_id) and admin_files_is_boxes = '0' and admin_files_name = '" . $filename . "'");
  if (tep_db_num_rows($dbquery)) {
	if ($submenu == 'submenu') {
...
function tep_has_product_freeshipping($products_id) {
        $attributes_query = tep_db_query("select offer_free_shipping from " . TABLE_PRODUCTS . " where products_id = '" . (int)$products_id . "'");
        $attributes = tep_db_fetch_array($attributes_query);
        return $attributes['offer_free_shipping'];
 }  
...
function tep_get_products_manufacture_url($product_id, $language = '') {
    global $languages_id;

    if (empty($language)) $language = $languages_id;

    $product_query = tep_db_query("select manufacturers_id from products where products_id = '" . (int)$product_id . "'");
    $product = tep_db_fetch_array($product_query);
     
	 //echo "select manufacturers_id from products where products_id = '" . (int)$product_id . "'";
	 
    $manufacture_query = tep_db_query("select manufacturers_url, offer_free_shipping ,manufacturers_name,manufacturers_id from manufacturers where manufacturers_id = '" . (int)$product['manufacturers_id'] . "'");
    $manufacture = tep_db_fetch_array($manufacture_query);
	
	//echo $manufacture['manufacturers_url'];
