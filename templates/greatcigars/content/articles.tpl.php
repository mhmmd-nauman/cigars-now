...
?>
    ...
    <?php
  } elseif ($topic_depth == 'articles' || isset($_REQUEST['authors_id'])) {
      ...
      <tr>
        <td><table border="0" width="100%" cellspacing="0" cellpadding="0">
           ...
           /* bof catdesc for bts1a, replacing "echo HEADING_TITLE;" by "topics_heading_title" */
             ...
             if (isset($_REQUEST['authors_id'])) {
               $author_query = tep_db_query("select au.authors_name, aui.authors_description, aui.authors_url from " . TABLE_AUTHORS . " au, " . TABLE_AUTHORS_INFO . " aui where au.authors_id = '" . (int)$_REQUEST['authors_id'] . "' and au.authors_id = aui.authors_id and aui.languages_id = '" . (int)$languages_id . "'");
...
// optional Article List Filter
    if (ARTICLE_LIST_FILTER) {
      ...
      if (tep_db_num_rows($filterlist_query) > 1) {
        ...
        if (isset($_REQUEST['authors_id'])) {
          echo tep_draw_hidden_field('authors_id', $_REQUEST['authors_id']);
        ...
        echo tep_draw_hidden_field('sort', $_REQUEST['sort']);
        while ($filterlist = tep_db_fetch_array($filterlist_query)) {
          $options[] = array('id' => $filterlist['id'], 'text' => $filterlist['name']);
        }
        echo tep_draw_pull_down_menu('filter_id', $options, (isset($_REQUEST['filter_id']) ? $_REQUEST['filter_id'] : ''), 'onchange="this.form.submit()"');
        echo '</form></td>' . "\n";
      }
    }
?>
