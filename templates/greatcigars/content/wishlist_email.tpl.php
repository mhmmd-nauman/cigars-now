...
<?php
    ...
    if (isset($_REQUEST['action']) && ($_REQUEST['action'] == 'process') && !tep_validate_email(trim($_POST['friendemail']))) {
    ...
    if (isset($_REQUEST['action']) && ($_REQUEST['action'] == 'process') && empty($_POST['friendname'])) {
    ...
    if (!tep_session_is_registered('customer_id')) {
      if (isset($_REQUEST['action']) && ($_REQUEST['action'] == 'process') && !tep_validate_email(trim($from_email_address))) {
    ...
    if (isset($_REQUEST['action']) && ($_REQUEST['action'] == 'process') && empty($from_name)) {
      $fromname_error = true;
      $error = true;
    } else {
    ...
    if (isset($_REQUEST['action']) && ($_REQUEST['action'] == 'process') && ($error == false)) {
...
<?php
      ...
      if (tep_session_is_registered('customer_id')) {
        ...
        $your_name_prompt = tep_draw_input_field('yourname', (($fromname_error == true) ? $_POST['yourname'] : $_REQUEST['yourname']));
        if ($fromname_error == true) $your_name_prompt .= '&nbsp;' . TEXT_REQUIRED;
        $your_email_address_prompt = tep_draw_input_field('from', (($fromemail_error == true) ? $_POST['from'] : $_REQUEST['from']));
        if ($fromemail_error == true) $your_email_address_prompt .= ENTRY_EMAIL_ADDRESS_CHECK_ERROR;
      }
?>
      <tr>
        <td><?php echo tep_draw_form('email_friend', tep_href_link(FILENAME_WISHLIST_SEND, 'action=process&products_id=' . $_REQUEST['products_id'])) . tep_draw_hidden_field('products_name', $product_info['products_name']); ?><table border="0" width="100%" cellspacing="0" cellpadding="2">
          <tr>
              ...
              <tr>
                <td class="main"><table border="0" cellspacing="0" cellpadding="2">
                  <tr>
                    ...
                    <td class="main"><?php echo tep_draw_input_field('friendname', (($friendname_error == true) ? $_POST['friendname'] : $_REQUEST['friendname'])); if ($friendname_error == true) echo '&nbsp;' . TEXT_REQUIRED;?></td>
                  </tr>
                  <tr>
                    <td class="main"><?php echo FORM_FIELD_FRIEND_EMAIL; ?></td>
                    <td class="main"><?php echo tep_draw_input_field('friendemail', (($friendemail_error == true) ? $_POST['friendemail'] : $_REQUEST['send_to'])); if ($friendemail_error == true) echo ENTRY_EMAIL_ADDRESS_CHECK_ERROR; ?></td>
                  </tr>
                </table></td>
              </tr>
            </table></td>
